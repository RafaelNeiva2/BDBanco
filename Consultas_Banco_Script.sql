...
-- 10 Essa consulta retorna as informações das pessoas que ainda não pagaram suas faturas do mês de julho
...
INNER JOIN Cartao Car 
	ON C.Cpf = Car.Cpf
INNER JOIN Fatura F 
	ON Car.IDcartao = F.IDcartao
WHERE Car.tipocartao = 'Crédito'and F.datavencimento > '2024-06-30'
ORDER BY F.datavencimento

-- Bonus

-- Retorne uma consulta onde se tenha a diferença entre a soma dos valores de depositos, e a soma dos valores de saques, em que não tenham relaçam com transferencia
...
-- Retorne nome do atendente, telefone, data de duvida, de todo atendimento cujo, referente a limite 
